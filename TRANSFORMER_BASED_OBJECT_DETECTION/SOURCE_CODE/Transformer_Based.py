...
def load_model(weights: str):
    try:
        ...
        sys.exit(f"Ultralytics is not installed (RTDETR class requires ultralytics >= 8.0.147).\nRun:  pip install -U ultralytics opencv-python\nOriginal error: {e}")
...
def run_detection(model, image_paths, conf_threshold: float, output_dir: Path):
    ...
    for idx, img_path in enumerate(image_paths, start=1):
        ...
        for box in r.boxes:
            ...
            color = (255, 0, 0) if is_victim_proxy else (200, 120, 0)
            label = f"{cls_name} {confidence:.2f}"
            if is_victim_proxy:
                label = f"VICTIM ({label})"

            cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)
            cv2.putText(img, label, (int(x1), max(0, int(y1) - 8)), cv2.FONT_HERSHEY_SIMPLEX, 3, color, 7)
...
def write_log(all_results, output_dir: Path):
    ...
    with open(summary_path, "w") as f:
        f.write("ASTRA - Transformer (RT-DETR) Object Detection Simulation Summary\n")
        f.write("=" * 60 + "\n")
        ...
        f.write(
            "\nNote: compare this file against the CNN (YOLOv8n) run's summary.txt\n"
            "on the same input folder/threshold for an apples-to-apples inference-\n"
            "time and detection-count comparison in the proposal.\n"
        )
...
def main():
    parser = argparse.ArgumentParser(description="ASTRA RGB-channel transformer-based object detection simulation (RT-DETR).")
    parser.add_argument("--input", required=True, help="Folder containing locally stored test images")
    parser.add_argument("--output", default="./detections_transformer", help="Folder to write annotated images + detection log (default: ./detections_transformer)")
    parser.add_argument("--model", default="rtdetr-l.pt", help="Ultralytics RT-DETR weights (default: rtdetr-l.pt, auto-downloads on first run; rtdetr-x.pt is a larger/slower alternative)")
